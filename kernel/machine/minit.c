...
static volatile int g_global_init_barrier = 0;
...
void m_start(uintptr_t hartid, uintptr_t dtb) {
  ...
  // global platform init should happen once.
  if (hartid == 0) {
  ...
  sync_barrier(&g_global_init_barrier, NCPU);
  if (hartid != 0) sprint("In m_start, hartid:%d\n", hartid);
