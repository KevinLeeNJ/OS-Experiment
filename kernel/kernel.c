...
static volatile int g_s_start_sync = 0;
...
void load_user_program(process *proc) {
  ...
  proc->trapframe->kernel_hartid = hartid;
...
int s_start(void) {
  ...
  // init phisical memory manager and kernel page table once.
  if (hartid == 0) {
    pmm_init();
    kern_vm_init();
  }
  sync_barrier(&g_s_start_sync, NCPU);

  // now, switch to paging mode by turning on paging (SV39)
  enable_paging();
  // the code now formally works in paging mode, meaning the page table is now in use.
  sprint("hartid = %ld: kernel page table is on \n", hartid);
