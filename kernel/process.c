...
int do_wait(int pid)
{
  ...
  for (int i = 0; i < NPROC; i++)
  {
    ...
    {
      // pid=-1则等待任意子进程结束，否则等待指定pid的子进程结束
      if (pid == -1 || (int)procs[i].pid == pid)
      {
        has_child = 1;
        // 如果子进程已经结束，回收资源并返回子进程pid
        if (procs[i].status == ZOMBIE)
        {
          int child_pid = procs[i].pid;
          procs[i].status = FREE;
          return child_pid;
        }
      }
...
void call_parent_from_blocked(process *proc)
{
  ...
  if (parent && parent->status == BLOCKED &&
      (parent->trapframe->regs.a0 == -1 ||
       parent->trapframe->regs.a0 == (int)proc->pid))
  {
    parent->status = READY;
